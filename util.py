...
def get_affine_theta(method, param=None, a_bound=None):
    ...
    if method == 'translation':
        ...
        a = 1 + np.random.uniform(a_bound[0], a_bound[1], 2) \
            if param is None else param
...
def random_affine_image(img, env_type, gen, phase, r_bound=[50, 60], sh_bound=[0.2, 0.5],
    ...
    """
    while True:
        rand = [randrange(2), randrange(2), randrange(4)+1]
        if phase == 'train':
            if rand in gen:
                continue
            else:
    ...
    if env_type == 'r':
        ...
        affine_mtx = t1_mtx @ r_mtx @ sc_mtx @ t2_mtx @ t3_mtx
    else:
...
def param2theta(param, env):
    ...
    if env in ['rsst']:
        ...
        t_mtx = theta2mtx(get_affine_theta('translation', param=param[3:5]))
    else:
        t_mtx = np.eye(3)
